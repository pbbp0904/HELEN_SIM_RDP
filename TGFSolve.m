function [x,y,z,xp,yp,zp,solver] = TGFSolve(A,ts,tgf)
...
try
    ...
    try
        ...
        equ1_2 = sqrt((x2-x0).^2+(y2-y0).^2+(z2-z0).^2)-sqrt(x0.^2+y0.^2+z0.^2) == c*t2;
        equ1_3 = sqrt((x3-x0).^2+(y3-y0).^2+(z3-z0).^2)-sqrt(x0.^2+y0.^2+z0.^2) == c*t3;
        equ1_4 = sqrt((x4-x0).^2+(y4-y0).^2+(z4-z0).^2)-sqrt(x0.^2+y0.^2+z0.^2) == c*t4;

        %Solve equations; Result in terms of t2,t3,t4; Two possible solutions
        solution=solve(equ1_2,equ1_3,equ1_4,x0,y0,z0);
